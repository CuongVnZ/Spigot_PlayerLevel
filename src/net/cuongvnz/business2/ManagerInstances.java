...
public class ManagerInstances {
    ...
    private static HashMap<Class<? extends AbstractManager>, PlayerLevel> associatedPlugin = new HashMap<Class<? extends AbstractManager>, PlayerLevel>();

    protected static final void registerManager(Class<? extends AbstractManager> clazz, AbstractManager instance, PlayerLevel pl) throws Exception {
    ...
    protected static final void unloadManager(Class<? extends AbstractManager> clazz) {
        if (instances.containsKey(clazz)) {
            ...
            PlayerLevel pl = associatedPlugin.remove(clazz);
            manager.unload(pl);
    ...
    protected static final void cleanup() {
        for (Entry<Class<? extends AbstractManager>, AbstractManager> e : instances.entrySet()) {
            PlayerLevel pl = associatedPlugin.get(e.getKey());
